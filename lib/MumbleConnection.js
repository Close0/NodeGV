...
MumbleConnection.prototype.disperse = function() {
	if(this.connectedToVS && this.sessionId) {
...
MumbleConnection.prototype.leaveServer = function(serverID) {
	if(this.connectedToVS && this.sessionId) {
		this.connectedToVS = false;
		this.sessionId = false;
		clearInterval( this.pingInterval );
		this.sendXMLMessage('MTYPE_CONN_serverDisconnect', {
			serverId: serverID
		});
	}
};
		...
		Messages.xmlToStructure(type,data, function(message){
			...
			switch(name){
				...
				case 'MTYPE_CONNECTION_ACTIVATED':
					self.emit('activated');
					break;
				case 'MTYPE_CONN_disconnectedFromVS':
					self.emit('disconnectedFromVS');
					break;
