...
var Messages = require('./GameVoxMessageMap');
...
MumbleConnection.prototype.FRAME_LENGTH = 10; //Audio per packet in GV land
...
MumbleConnection.prototype.authenticate = function( name, password ) {
	this.sendXMLMessage('MTYPE_CONNECT_AUTHENTICATE', {
		username: name,
		password: password,
		protocolVersion: 44,
		version: '0.11.1337'
...
/**
 * Activate this connection to send and recieve data
 *
 * @param name Username
 **/
MumbleConnection.prototype.activate = function(needAccountData) {
	this.sendXMLMessage('MTYPE_CONNECTION_ACTIVATE', {
		needAccountData: needAccountData,
	});
...
/**
 * Send a XML protocol message
 *
 * @param type Message type ID
 * @param data Message data
 **/
MumbleConnection.prototype.sendXMLMessage = function (type, data) {
	// Look up the message schema by type and serialize the data into protobuf format.
	//var msg = Messages.schemaByName[type].serialize(data);

	var xml = builder.create("value", { headless: true }).ele('struct');
	for(var name in data){
		var member = xml.ele('member');
			member.ele('name',name);
			member.ele('value').ele(Messages.getXMLStructure(type)[name] || "undefined",data[name]);
	}
	var msg = xml.end();
...
//Join a random room
MumbleConnection.prototype.disperse = function() {
	this.sendMessage( 'UserState', {
		session: this.sessionId,
		channelId: Math.floor((Math.random() * this.channels.length) + 1)
	});
};

//Join the root channel
MumbleConnection.prototype.assemble = function() {
	this.sendMessage( 'UserState', {
		session: this.sessionId,
		channelId: 0
	});
...
MumbleConnection.prototype.sendVoiceFrame = function( frame ) {
	...
	if( !this.connectedToVS ) { throw new Error( 'Wait for "connectedToVS" event before attempting to send audio.' ); }
...
MumbleConnection.prototype.sendEncodedFrame = function ( packets ) {
	...
	var sessionID = Buffer(2);
	sessionID.writeUInt16BE(this.sessionId,0);

	//Increase IV
	for(var k = 0; k < this.udpDetails.clientNonce.length; k++) {
		this.udpDetails.clientNonce[k]++;
		if(this.udpDetails.clientNonce[k]){
			break;
		}
	}

	// munge the key
	var mungedKey = new Buffer(16);
	for(var j = 0; j < this.udpDetails.key.length; ++j) {
		mungedKey[j] = this.udpDetails.key[j] ^ this.udpDetails.clientNonce[j];
	}

	//insert counter
	var counter = new Buffer([this.udpDetails.clientNonce[0]]);


	//console.dir(this.udpDetails.key);
	//console.dir(mungedKey);

	var canary = new Buffer([0x55,0x55]);

	var frameBuffer = Buffer.concat(frames);
	var frameBufferWithCanary = Buffer.concat([frameBuffer,canary]);
	//var frameBufferWithCounter = Buffer.concat([counter,frameBuffer]);

	// encrypt the data
	//console.log("Pre encryption length=" +  frameBufferWithCanary.length);
	var cipher = crypto.createCipheriv('AES-128-CBC',mungedKey,this.udpDetails.clientNonce);
	var encryptedData = Buffer.concat([
										cipher.update(frameBufferWithCanary),
										cipher.final()
									]);
	//console.log("Post encryption length=" +  encryptedData.length);

/*** TEMPORARY ***/
	//var decrypterificationizer = crypto.createDecipheriv('AES-128-CBC',mungedKey,this.udpDetails.clientNonce);
	//decrypterificationizer.update(encryptedData);
	//console.dir(decrypterificationizer.final());
/*** TEMPORARY ***/

	// generate MAC
	var counterWithEncryptedData = Buffer.concat([counter,encryptedData]);
	var hmac = crypto.createHmac('md5',this.udpDetails.macKey);
	hmac.update(counterWithEncryptedData);
	var digest = hmac.digest();

	var finalBuffer = Buffer.concat([sessionID,counter,encryptedData,digest]);

	var udp = this.udp;
	udp.send(finalBuffer, 0, finalBuffer.length, this.udpDetails.udpPort, this.udpDetails.udpAddress, function(err, bytes) {
		if(err) {
	...
	//for( var f in frames ) {
	//	var udp = this.udp;
	//	var encryptedPacket = frames[f];
	//	var cipher = crypto.createCipheriv('aes128',this.udpDetails.key,this.udpDetails.clientNonce);
	//	cipher.update(encryptedPacket);
	//	encryptedPacket = cipher.final();
	//	udp.send(encryptedPacket, 0, encryptedPacket.length, this.udpDetails.udpPort, this.udpDetails.udpAddress, function(err, bytes) {
	//		udp.close();
	//	});
	//}
};

MumbleConnection.prototype.joinServer = function(serverID) {
	this.sendXMLMessage('MTYPE_CONN_serverConnect', {
		serverId: serverID,
		password: ''
	});
};

MumbleConnection.prototype.serverSync = function() {
	this.sendMessage('ServerSync',{});
};

MumbleConnection.prototype.generateSound = function() {
	/**/
	...
	setInterval(function() {
		...
		var voiceData = new Buffer(7);
		voiceData.writeUInt8(128, 0); //TODO: Defined in Message.h::MessageHandler::UDPMessageType::UDPVoiceOpus
		//voiceData.writeInt32BE(self.sessionId, 1);
		voiceData.writeInt32BE(++sequenceNumber, 1); //Sequence
		voiceData.writeUInt8(64, 5); //Audio header, Bit 1: Terminator, Bit 2-8: Data length
		voiceData.writeUInt8(0x42, 6);
		//voiceData.writeInt8(0x42, 7);
		//voiceData.writeInt8(0x42, 8);

		var prefix = new Buffer(6);
		prefix.writeInt16BE(0x00, 0);
		prefix.writeInt32BE(voiceData.length, 2);


		var packet = new Buffer.concat([prefix,voiceData]);
		console.log("SessionID: " + self.sessionId + ", Type=" + (typeof self.sessionId));
		console.dir(packet);
...
MumbleConnection.prototype._processData = function (type, data) {
	if(!Messages.nameById[type]){
		console.warn("Undefined message type: 0x" + type.toString(16));
		return;
	}
	var self = this;
	if(Messages.isXMLMessage(type)){
		//Clean up the data into xml or null
		Messages.xmlToStructure(type,data, function(message){
			var name = Messages.nameById[type] || "UNKNOWN";
			switch(name){
				case 'MTYPE_CONN_friendUpdate':
					console.log("***Friend update: UserID=%s,Server=%s,isOnline=%s,Sequence=%s", message.UID,message.SID,message.isOnline,message.sequence);
					break;
				case 'MTYPE_CONNECT_AUTHENTICATED':
					self.emit('authenticated', message);
					break;
				case 'MTYPE_CONNECTION_ACTIVATED':
					self.emit('activated');
					break;
				case 'MTYPE_CONN_connectedToVS':
					// Set up the ping loop.
					if(!self.connectedToVS) {
						self.pingInterval = setInterval(function () {
							self._ping();
						}, 5000);
					}
					self.connectedToVS = true;
					self.emit('connectedToVS', message);
					break;
				default:
					//console.warn("Unhandled XML Message: " + name);
					break;
			}
		});
	} else {
		...
		switch(Messages.nameById[type] || 0){
			case 'UDPPacket':
			...
			case 'ServerSync':
				...
				break;
			default:
				console.warn("Unhandled protobuf type: 0x" + type.toString(16) + " (" + Messages.nameById[type] + ")");
				//console.dir(msg);
				break;
		}
	}
...
MumbleConnection.prototype._onServerSync = function ( syncData ) {
	this.channels = syncData.channels;
	this.sessionId = syncData.session;
	this.udpDetails = syncData.cryptSetup;
	this.emit('serverSync',syncData);
};
