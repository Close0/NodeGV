...
MumbleConnection.prototype.sendEncodedFrame = function ( packets ) {
	...
	var mundgedKey = new Buffer(16);
	for(var j = 0; j < this.udpDetails.clientNonce.length; ++j) {
		mundgedKey[j] = this.udpDetails.key ^ this.udpDetails.clientNonce[j];
	}

	console.dir(this.udpDetails.key);
	console.dir(mundgedKey);

	var frameBuffer = Buffer.concat(frames);
	var cipher = crypto.createCipheriv('aes128',mundgedKey,this.udpDetails.clientNonce);
	var hmac = crypto.createHmac('md5',this.udpDetails.macKey);
	cipher.update(frameBuffer);
	frameBuffer = cipher.final();
	hmac.update(frameBuffer);
	var digest = hmac.digest();

	var finalBuffer = Buffer.concat([buffer,frameBuffer,digest]);
