...
MumbleConnection.prototype.authenticate = function( name, password ) {
    this.sendXMLMessage('Authenticate', {
        username: name,
        password: password,
        protocolVersion: 44,
        version: '0.11.1337'
...
MumbleConnection.prototype.sendXMLMessage = function (type, data) {
    ...
    for(var name in data){
        var member = xml.ele('member');
            ...
            member.ele('value').ele(Messages.idByName[type].structure[name] || "undefined",data[name]);
    }
    var msg = xml.end();

    // Create the prefix.
    var prefix = new Buffer(6);
    prefix.writeUInt16BE(Messages.idByName[type].constant, 0);
    prefix.writeUInt32BE(msg.length, 2);
