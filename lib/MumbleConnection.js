...
MumbleConnection.prototype.sendEncodedFrame = function ( packets ) {
	...
	for(var j = 0; j < this.udpDetails.key.length; j++) {
		mungedKey[j] = this.udpDetails.key ^ this.udpDetails.clientNonce[j];
	}
	...
	frameBuffer = Buffer.concat([frameBuffer,canary]);
	//var frameBufferWithCounter = Buffer.concat([counter,frameBuffer]);

	// encrypt the data
	var cipher = crypto.createCipheriv('aes128',mungedKey,this.udpDetails.clientNonce);
	...
	if(Messages.isXMLMessage(type)){
		...
		switch(Messages.nameById[type] || 0){
			...
			case 'ServerSync':
				...
				break;
			default:
				//console.warn("Unhandled protobuf type: 0x" + type.toString(16));
				//console.dir(msg);
				break;
		}
	}
